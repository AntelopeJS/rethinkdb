...
export interface Datum<T = any> extends Query<T> {
   ...
   */
  <U extends keyof T>(attr: U): Datum<T[U]>;
  /**
...
export interface Stream<T = any> extends Query<T[]> {
   ...
   */
  orderBy(field: keyof T, direction?: 'asc' | 'desc', noIndex?: boolean): this;
  /**
...
export interface SingleSelection<T = any> extends Datum<T> {
   ...
   */
  update(obj: Value<DeepPartial<T>> | ((arg: Datum<T>) => any), options?: Options.Update): Query<Result.Write<T>>;
  /**
   ...
   * @returns Write result
   */
  replace(obj: Value<T> | ((arg: Datum<T>) => any), options?: Options.Update): Query<Result.Write<T>>;
  /**
   * Delete the document.
   *
